...
For more information see the `convenience functions <get_input_convenience.html>`_
documentation,
...
`Cooked_input` has a lot more features and capabilities for getting, cleaning and validating inputs.  To learn about the advanced
capabilities of `cooked_input`, see the `tutorial <tutorial.html>`_ and `how to <how_to.html>`_.

`Cooked_input` also has c companion package `Cooked_table` for input from menus and tables.  To learn more about `cooked_table` see 
http://cooked-table.readthedocs.io/en/latest/
