...
def build_a_burger_v1(conn):
    ...
    tis = [ci.TableItem(row[1:]) for row in cursor]
    tbl = ci.Table(tis, col_names=col_names.split(), prompt=prompt_str, default_choice=def_choice,
                   add_exit=False, default_action='row')
